...
CREATE INDEX idx_policy_acceptances_order ON policy_acceptances(order_id, user_id) WHERE order_id IS NOT NULL;

-- Archiving a published version is allowed. Altering accepted content is not.
CREATE TRIGGER policy_documents_immutable_update
...
CREATE TRIGGER policy_documents_immutable_delete
