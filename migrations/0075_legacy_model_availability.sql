...
UPDATE inventory_ledger SET scope_id=(SELECT 'ful_'||a.option_id||'_'||a.fulfillment_type FROM product_option_aliases a WHERE a.legacy_option_id=inventory_ledger.scope_id),scope='fulfillment'
...
UPDATE product_option_values SET availability_type='', stock=NULL,reserved=0,
 ...
 name_en=CASE WHEN variant_label<>'' THEN variant_label ELSE trim(CASE WHEN lower(name_en) LIKE '% — pre-order' THEN substr(name_en,1,length(name_en)-12) WHEN lower(name_en) LIKE '% — direct' THEN substr(name_en,1,length(name_en)-9) WHEN lower(name_en) LIKE '% - pre-order' THEN substr(name_en,1,length(name_en)-12) WHEN lower(name_en) LIKE '% - direct' THEN substr(name_en,1,length(name_en)-9) WHEN lower(name_en) LIKE '% — طلب مسبق' THEN substr(name_en,1,length(name_en)-11) WHEN lower(name_en) LIKE '% — بيع مباشر' THEN substr(name_en,1,length(name_en)-12) WHEN lower(name_en) LIKE '% - طلب مسبق' THEN substr(name_en,1,length(name_en)-11) WHEN lower(name_en) LIKE '% - بيع مباشر' THEN substr(name_en,1,length(name_en)-12) ELSE name_en END ) END ,
 name_ar=trim(CASE WHEN lower(name_ar) LIKE '% — pre-order' THEN substr(name_ar,1,length(name_ar)-12) WHEN lower(name_ar) LIKE '% — direct' THEN substr(name_ar,1,length(name_ar)-9) WHEN lower(name_ar) LIKE '% - pre-order' THEN substr(name_ar,1,length(name_ar)-12) WHEN lower(name_ar) LIKE '% - direct' THEN substr(name_ar,1,length(name_ar)-9) WHEN lower(name_ar) LIKE '% — طلب مسبق' THEN substr(name_ar,1,length(name_ar)-11) WHEN lower(name_ar) LIKE '% — بيع مباشر' THEN substr(name_ar,1,length(name_ar)-12) WHEN lower(name_ar) LIKE '% - طلب مسبق' THEN substr(name_ar,1,length(name_ar)-11) WHEN lower(name_ar) LIKE '% - بيع مباشر' THEN substr(name_ar,1,length(name_ar)-12) ELSE name_ar END ),name_ckb=trim(CASE WHEN lower(name_ckb) LIKE '% — pre-order' THEN substr(name_ckb,1,length(name_ckb)-12) WHEN lower(name_ckb) LIKE '% — direct' THEN substr(name_ckb,1,length(name_ckb)-9) WHEN lower(name_ckb) LIKE '% - pre-order' THEN substr(name_ckb,1,length(name_ckb)-12) WHEN lower(name_ckb) LIKE '% - direct' THEN substr(name_ckb,1,length(name_ckb)-9) WHEN lower(name_ckb) LIKE '% — طلب مسبق' THEN substr(name_ckb,1,length(name_ckb)-11) WHEN lower(name_ckb) LIKE '% — بيع مباشر' THEN substr(name_ckb,1,length(name_ckb)-12) WHEN lower(name_ckb) LIKE '% - طلب مسبق' THEN substr(name_ckb,1,length(name_ckb)-11) WHEN lower(name_ckb) LIKE '% - بيع مباشر' THEN substr(name_ckb,1,length(name_ckb)-12) ELSE name_ckb END ),
...
UPDATE products SET options=(SELECT json_group_array(json_object('id',v.id,'name_en',v.name_en,'name_ar',v.name_ar,'name_ckb',v.name_ckb,'active',json(CASE v.active WHEN 1 THEN 'true' ELSE 'false' END ),'image',v.image,'variant_key',v.variant_key,'regular_price_iqd',v.regular_price_iqd,'prime_price_iqd',v.prime_price_iqd,'pro_price_iqd',v.pro_price_iqd,'cost_iqd',v.cost_iqd)) FROM product_option_values v WHERE v.product_id=products.id)
