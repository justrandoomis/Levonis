...
WHEN EXISTS(SELECT 1 FROM json_tree(CASE WHEN json_valid(NEW."avatar_key") THEN NEW."avatar_key" ELSE json_array(NEW."avatar_key") END) scalar JOIN media_cleanup_locks locked ON substr(scalar.value,1,instr(scalar.value||'?','?')-1)=locked.object_key OR substr(scalar.value,1,instr(scalar.value||'?','?')-1)='/files/'||locked.object_key OR (substr(scalar.value,1,instr(scalar.value||'?','?')-1) LIKE 'https://%' AND substr(substr(scalar.value,1,instr(scalar.value||'?','?')-1),-length('/files/'||locked.object_key))='/files/'||locked.object_key) WHERE scalar.type='text') OR
