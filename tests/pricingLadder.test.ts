...
test('an explicit PRO price still wins over the PRIME fallback, and the store-wide PRO policy still applies before it', () => {
  ...
  // 10% off 150,000 is 135,000 — WORSE than the 125,000 the owner typed for
  // PREMIUM on this product, so the ladder inverts and has to be resolved.
  assert.equal(policy.prime_iqd, 125_000, 'the typed PREMIUM price, untouched');
  assert.equal(policy.pro_iqd, 125_000, 'and PRO never pays more than PREMIUM');
  assert.equal(policy.member_rule.pro, null, 'the rule was overruled, so it is not credited');
});

/**
 * A GENERATED PRICE NEVER RAISES A TYPED ONE.
 *
 * Until benefit rules existed, both member prices were numbers one person
 * typed on one screen, and an inversion between them was a data fault the
 * resolver settled by clamping PREMIUM UP to PRO. That is no longer safe: a
 * PRO price can now be GENERATED by a store-wide rule against a product whose
 * PREMIUM price somebody typed deliberately — a clearance item is exactly that
 * shape — and clamping up would charge that PREMIUM member MORE because of a
 * rule written for the whole catalogue.
 *
 * So when exactly one side comes from a rule, the rule-derived side is the one
 * that moves. Both typed keeps the historical clamp: it really is a data fault.
 */
test('a weaker PRO rule is lowered to the typed PREMIUM price, never the other way round', () => {
  const r = resolveUnitPrice({
    product: product({ price_iqd: 1_000, prime_price_iqd: 800, pro_price_iqd: null }),
    tier: 'prime',
    tierActive: true,
    proPolicy: { mode: 'global_percent', percent: 5 },
  });
  assert.equal(r.prime_iqd, 800, 'the owner typed 800 and 800 is what PREMIUM pays');
  assert.equal(r.applied_iqd, 800);
  assert.equal(r.pro_iqd, 800, 'PRO gets the better of the two, as the ladder promises');

  // Both typed: the historical clamp, because there is no rule to blame.
  assert.deepEqual(clampMemberLadder(1_000, 800, 950), { prime: 950, pro: 950 });
  // PRO derived, PREMIUM typed: the derived side moves.
  assert.deepEqual(clampMemberLadder(1_000, 800, 950, { pro: true }), { prime: 800, pro: 800 });
  // PREMIUM derived, PRO typed: the derived side moves — the typed PRO stands.
  assert.deepEqual(clampMemberLadder(1_000, 800, 950, { prime: true }), { prime: 950, pro: 950 });
});
