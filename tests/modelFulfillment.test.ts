...
test('the identical TXT can be imported again after permanent deletion without stale IDs or slug conflicts', async () => {
  const raw = freshDb(); const db = asD1(raw);
  const env = { DB: db, BUCKET: new MemoryMedia(), APP_ORIGIN: 'https://levonis-iq.com' } as unknown as Env;
  const app = stubApp(db, { id: 'owner', role: 'admin', email: 'owner@test.com' }, a => a.route('/api/admin/template', templateRoutes));
  const imported = await post(app, '/api/admin/template/apply', { text: FULFILLMENT_TXT, mode: 'draft', confirm: true });
  const first = await json(imported); assert.equal(imported.status, 200, JSON.stringify(first));
  await deleteProductPermanently(env, first.product_id as string, 'owner');
  const reimported = await post(app, '/api/admin/template/apply', { text: FULFILLMENT_TXT, mode: 'draft', confirm: true });
  const second = await json(reimported); assert.equal(reimported.status, 200, JSON.stringify(second));
  assert.notEqual(second.product_id, first.product_id);
  assert.equal(row(raw, 'SELECT COUNT(*) n FROM products WHERE slug=?', 'a1-mini-model-test')?.n, 1);
  ...
  assert.equal(row(raw, 'SELECT COUNT(*) n FROM product_option_transports')?.n, 2);
});

test('published TXT examples import and resolve their documented model, direct and transport prices', async () => {
  const raw = freshDb(); const db = asD1(raw);
  const app = stubApp(db, { id: 'owner', role: 'admin', email: 'owner@test.com' }, a => a.route('/api/admin/template', templateRoutes));
  for (const [filename, checks] of [
    ['a1-mini-fulfillment.txt', [['a1-mini', null, 549000], ['a1-mini', 'air', 549000], ['a1-mini-combo', null, 699000], ['a1-mini-combo', 'air', 759000], ['a1-mini-combo', 'land', 709000]]],
    ['printer-a-fulfillment.txt', [['printer-a', null, 1050000], ['printer-a', 'sea', 1000000], ['printer-a', 'land', 1030000], ['printer-a', 'air', 1080000]]],
  ] as const) {
    const text = readFileSync(new URL(`../docs/examples/${filename}`, import.meta.url), 'utf8');
    const result = await post(app, '/api/admin/template/apply', { text, mode: 'draft', confirm: true });
    const body = await json(result); assert.equal(result.status, 200, JSON.stringify(body));
    const id = body.product_id as string;
    const doc = applyRelations(parseProductRow(row(raw, 'SELECT * FROM products WHERE id=?', id)!), await loadRelationsView(db, id, 'BASE'));
    for (const [model, transport, expected] of checks) assert.equal(quote(doc, model, transport).unit_subtotal_iqd, expected, `${filename}: ${model}/${transport}`);
  }
});
