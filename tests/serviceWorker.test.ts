...
/**
 * THE PRECACHED BYTES AND THE VERSION THEY SHIPPED UNDER, PINNED TOGETHER.
 *
 * WHAT THIS CLOSES, because it is a bug that already shipped and cost the
 * owner a logo they thought they had replaced. `PRECACHE_URLS` is the one list
 * in the worker whose entries are not content-hashed: `/icons/icon-192.png`
 * keeps its name for ever while the bytes behind it are regenerated by hand
 * from the shop logo (scripts/build-pwa-icons.mjs; src/lib/siteLogo.ts calls
 * those seven PNGs a fork, because no upload to R2 reaches them). Of the seven
 * only apple-touch-icon and the two favicons are linked from the document, so
 * only those three ever take the stale-while-revalidate refresh. `icon-192`,
 * `icon-512` and the two maskables are read by the install prompt and by
 * nothing else — once they sit in `levonis-static-<VERSION>` the only thing
 * that can evict them is `activate` finding that cache name no longer current.
 *
 * So regenerating the icons WITHOUT bumping VERSION is silent: the repository,
 * the manifest and every fresh visitor get the new mark, and every already
 * installed app keeps repainting the old one out of its own precache, for
 * ever. That is what v1 did — 367d2d1 replaced all seven PNGs after v1 had
 * shipped and the constant stayed put.
 *
 * Pinning the pair is what makes it loud. Change the icon bytes and this test
 * fails until `public/sw.js` is bumped and both constants below are updated in
 * the same commit. A bump made for some other reason fails it too, and the fix
...
test('the precached icon bytes and the worker version move together', () => {
  ...
  // Name and bytes both, in list order: renaming an entry has to count as a
  // change even when the seven files are byte-for-byte the ones already there.
  const hash = createHash('sha256');
  for (const url of urls) {
    hash.update(url);
    hash.update('\0');
    hash.update(readFileSync(join(ROOT, 'public', url.slice(1))));
  }

  assert.deepEqual(
    { version: sw.internals.VERSION, digest: hash.digest('hex') },
    { version: PRECACHE_VERSION, digest: PRECACHE_DIGEST },
    'the precached icons or the worker version changed on their own. They are one thing: ' +
      'new icon bytes need a new VERSION in public/sw.js, or every installed app keeps ' +
      'drawing the old mark from levonis-static-' + PRECACHE_VERSION + '. Bump it, then update both ' +
