...
| `bambu-project-3mf.ts` | The output that SHIPS: converts the engine's 3MF into a Bambu-shaped project (schema markers, the `project_settings.config` header, plate thumbnails, `slice_info.config`, upstream's `_rels`/`[Content_Types]`). Derived from bbs_3mf.cpp; deliberately does not claim Bambu Studio provenance. |
| `makerworld/` (`preflight.ts`, `prepare.tsx`), `export-manager.ts` | A DESIGNED output flow (S7) that is **not mounted** — nothing in `app/` imports it. The manager's state machine is unit-tested; the UI over it is not shipped: preflight → real 3MF via the engine save path (structurally verified) → transfer summary → open the official MakerWorld upload page. Honest wording only — "file ready" / "open MakerWorld", never "uploaded"; no sanctioned third-party upload API exists. G-code download is a secondary action. |
