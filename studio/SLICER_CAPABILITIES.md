...
| General Auto Arrange | Implemented | Shelf packing across the engine's real 9-plate cap, locked objects respected, oversized/overflow reported rather than force-fitted, with a full-scene undo. |
| Auto Orient | Implemented | BambuStudio's `AutoOrienter` scoring ported to TypeScript (`app/auto-orient.ts`): candidate directions from face-area cumulation over the mesh and its 3D convex hull plus 18 supplements, scored on overhang, bed contact, hull footprint and low-angle faces with upstream's `OrientParamsArea` constants. Applied through the engine's own `sceneSnapshot`/`restoreScene`, so no engine patch is involved; undoable. `tests/auto-orient.test.mjs` covers 15 cases. Physical verification is outstanding. |
| Plane cut | Implemented | Horizontal cut through the selected object at a height bounded by its own extent, with the cross-section closed (holes preserved) and both pieces re-seated on the bed. Applied through the engine's `sceneSnapshot`/`restoreScene`, which creates objects from new geometry, so no engine patch is involved; undoable. The operation follows BambuStudio's `cut_mesh`; the geometry is this repository's own, tested on closed-solid and conserved-volume properties (`tests/plane-cut.test.mjs`, 9 cases). Arbitrary cut planes, Boolean operations and modifier parts remain unavailable. |
| Boolean / modifier parts | Missing | Visible as disabled native controls; the web engine lacks the part implementation, and Boolean needs a robust CSG kernel this does not ship. |
