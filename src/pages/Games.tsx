...
export default function Games() {
  ...
  return (
    <div className="w-full min-h-screen bg-white dark:bg-black font-sans pb-10 pt-16">
      ...
      {/* Game List — games have not launched yet, so cards are previews only */}
      <div className="px-4 flex flex-col gap-6 mt-2">
        {games.map((game) => (
          <div key={game.id} className="relative">
             <div
               aria-disabled="true"
               className={`relative w-full rounded-[24px] overflow-hidden ${game.bg} shadow-lg ${game.shadow} opacity-80 cursor-not-allowed`}
             >
                ...
                <div className="absolute bottom-0 left-0 right-0 px-4 py-2 bg-white/20 backdrop-blur-[2px] flex justify-between items-center z-10">
                   <div></div>
                   <span className="text-white font-bold text-[10px] tracking-wider uppercase opacity-90">قريباً / Coming soon</span>
                </div>
             </div>
          </div>
        ))}
