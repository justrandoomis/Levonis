...
export default function Invest() {
  ...
  if (loading) {
    return (
      <div className="w-full min-h-screen bg-black flex items-center justify-center" dir="ltr">
        <div className="w-6 h-6 border-2 border-apricot border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
  ...
  if (accessDenied) {
    return (
      <div className="w-full min-h-screen bg-black text-zinc-100 font-sans flex flex-col items-center justify-center gap-4 p-8 text-center" dir="ltr">
        ...
        <button
          onClick={() => navigate('/')}
          className="mt-2 bg-[#e6a84f] hover:bg-[#d49942] text-snow font-bold px-8 py-3 rounded-full transition-colors"
        >
    ...
    return (
      <div className="w-full min-h-screen bg-black text-zinc-100 font-sans flex flex-col items-center justify-center gap-4 p-8 text-center" dir="ltr">
        ...
        <button
          ...
          className="bg-zinc-900 hover:bg-zinc-800 text-zinc-200 font-bold px-8 py-3 rounded-full transition-colors"
        >
  ...
  return (
    <div className="w-full min-h-screen bg-black text-zinc-100 font-sans flex flex-col" dir="ltr">
...
function NavItem({ icon, label, active, onClick }: { icon: any, label: string, active: boolean, onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${active ? 'text-apricot' : 'text-zinc-500'}`}>
  ...
  return (
    <div className="w-full flex flex-col pt-6">
      <div className="flex justify-end items-center px-6 mb-8 relative">
        <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="w-10 h-10 bg-zinc-900 rounded-full flex items-center justify-center">
          <User className="w-6 h-6 text-zinc-200" />
        </button>
        {showProfileMenu && (
          <div className="absolute right-6 top-12 bg-black border border-zinc-800 shadow-xl rounded-xl p-4 flex flex-col gap-4 z-50 min-w-[200px]">
            <div>
              <label className="text-xs font-bold text-zinc-500 uppercase mb-2 block">Language</label>
              <div className="flex gap-2">
                <button onClick={() => setLang('en')} className={`px-3 py-1 rounded-full text-sm ${lang === 'en' ? 'bg-[#e6a84f] text-snow font-bold' : 'bg-zinc-900 text-zinc-400'}`}>EN</button>
                <button onClick={() => setLang('ar')} className={`px-3 py-1 rounded-full text-sm ${lang === 'ar' ? 'bg-[#e6a84f] text-snow font-bold' : 'bg-zinc-900 text-zinc-400'}`}>AR</button>
              </div>
            </div>
            <div>
              <label className="text-xs font-bold text-zinc-500 uppercase mb-2 block">Currency</label>
              <div className="flex gap-2">
                <button onClick={() => setInvestCurrency('IQD')} className={`px-3 py-1 rounded-full text-sm ${investCurrency === 'IQD' ? 'bg-[#e6a84f] text-snow font-bold' : 'bg-zinc-900 text-zinc-400'}`}>IQD</button>
                <button onClick={() => setInvestCurrency('USD')} className={`px-3 py-1 rounded-full text-sm ${investCurrency === 'USD' ? 'bg-[#e6a84f] text-snow font-bold' : 'bg-zinc-900 text-zinc-400'}`}>USD</button>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex flex-col items-center justify-center text-center px-6 mb-8 mt-4">
        ...
        <div className="mb-4 flex justify-center">
          <span className="text-5xl font-extrabold tracking-tight text-zinc-100" style={{ direction: 'ltr' }}>{formatCurrency(totalProfit)}</span>
        </div>
        <div className="flex items-center gap-2 text-sm font-medium text-zinc-500 bg-zinc-900 px-4 py-2 rounded-full">
          <span>{tInvest('Total Invested:', 'إجمالي الاستثمارات:')}</span>
          <span className="text-zinc-100 font-bold">{formatCurrency(totalInvested)}</span>
        </div>
      </div>
  ...
  return (
    <div className="w-full flex flex-col pt-12 px-6">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-bold text-zinc-100 mx-auto">{tInvest('Invest', 'استثمار')}</h1>
      </div>
      ...
      <div className="bg-black rounded-2xl border border-zinc-800 p-5 mb-6 flex justify-between items-center shadow-sm">
         <div className="flex flex-col">
           <span className="text-zinc-500 text-xs font-bold uppercase mb-1">{tInvest('Invested Amount', 'المبلغ المستثمر')}</span>
           <span className="font-bold text-lg text-zinc-100">{formatCurrency(totalInvested)}</span>
         </div>
         <div className="h-10 w-[1px] bg-zinc-800 mx-4"></div>
      ...
      <div className="bg-black rounded-2xl border border-zinc-800 p-6 mb-6 shadow-sm">
        <h3 className="font-bold text-lg text-zinc-100 mb-6">{tInvest('Your Investments', 'استثماراتك')}</h3>
        ...
        {investments.length === 0 ? (
          ...
          <div className="flex flex-col gap-6">
            {investments.map((inv) => {
              ...
              return (
                <div key={inv.id} onClick={() => setSelectedInvest(inv)} className="flex justify-between items-center cursor-pointer p-4 hover:bg-zinc-950 rounded-xl transition-colors border border-zinc-800">
                  <div className="flex flex-col">
                    <span className="font-bold text-zinc-100">{tInvest('Investment', 'استثمار')} #{inv.id.slice(-4).toUpperCase()}</span>
                    ...
                    {isCancelled ? (
                      ...
                      <span className="text-xs text-apricot font-bold">{tInvest('Active', 'نشط')}</span>
                    )}
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="font-bold text-zinc-100">{formatCurrency(isCancelled ? inv.amount_usd_cents : currentVal)}</span>
                    {!isCancelled && (
      ...
      <div className="bg-zinc-950 rounded-2xl border border-zinc-800 p-6 mt-2 text-center">
        ...
        <button onClick={goToSupport} className="bg-[#e6a84f] hover:bg-[#d49942] text-snow font-bold px-8 py-3 rounded-full transition-colors">
  ...
  return (
    <div className="w-full flex flex-col pt-12 px-6">
      <div className="flex items-center mb-8 relative">
        ...
        <h1 className="text-xl font-bold text-zinc-100 mx-auto">{tInvest('Investment Details', 'تفاصيل الاستثمار')}</h1>
      </div>

      <div className="bg-black rounded-2xl border border-zinc-800 p-6 mb-6">
        <h2 className="text-3xl font-bold text-zinc-100 mb-2">{formatCurrency(currentVal)}</h2>
        <div className="flex flex-col gap-2 mb-6">
          ...
          <div className="flex justify-between items-center text-sm border-t border-zinc-800 pt-2">
            <span className="text-zinc-600">{tInvest('Accrued Profit:', 'الربح المتراكم حتى الآن:')}</span>
      ...
      <h3 className="font-bold text-lg text-zinc-100 mb-4">{tInvest('Purchased Items', 'العناصر المشتراة')}</h3>
      <div className="flex flex-col gap-4">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-4 bg-black p-4 rounded-xl border border-zinc-800 shadow-sm">
            {item.image ? (
...
function ChatTab({ messages, loadData, tInvest }: {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-black">
      <div className="flex items-center p-4 bg-black border-b border-zinc-800">
        <div className="w-10 h-10 bg-zinc-900 rounded-full flex items-center justify-center mr-3">
          <User className="w-6 h-6 text-zinc-500" />
        </div>
        <div className="flex flex-col">
          <span className="font-bold text-zinc-100">{tInvest('Support', 'الدعم')}</span>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        ...
        {messages.map((m) => (
          <div key={m.id} className={`max-w-[80%] p-3 rounded-2xl ${m.sender === 'user' ? 'bg-zinc-200 text-black self-end rounded-tr-sm font-medium' : 'bg-zinc-900 text-zinc-100 self-start rounded-tl-sm border border-zinc-800'}`}>
            {m.message}
          </div>
        ))}
      ...
      <div className="p-4 bg-black border-t border-zinc-800">
        <div className="flex items-center bg-black border border-zinc-700 rounded-full px-4 py-2">
          <input
            type="text"
          ...
          />
          <button onClick={send} disabled={sending || !text.trim()} className="ml-2 text-zinc-100 disabled:opacity-40">
...
function MoveFundsTab({ tInvest }: { tInvest: (en: string, ar: string) => string }) {
  ...
  return (
    <div className="w-full flex flex-col pt-12 px-6 pb-24">
      <h1 className="text-xl font-bold text-zinc-100 mx-auto mb-8">{tInvest('Move funds', 'نقل الأموال')}</h1>

      <div className="flex flex-col items-center justify-center text-center gap-4 py-12">
        <div className="w-16 h-16 rounded-full bg-zinc-900 flex items-center justify-center">
          <WalletIcon className="w-8 h-8 text-apricot" />
        </div>
        <h2 className="text-lg font-bold text-zinc-100">{tInvest('Deposits & Withdrawals', 'الإيداع والسحب')}</h2>
        <p className="text-sm text-zinc-500 max-w-sm">
          {tInvest(
