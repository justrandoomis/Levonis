...
export default function Bundles() {
  ...
  useEffect(() => {
    async function fetchProducts() {
      setLoading(true);
      try {
        let sql = "SELECT * FROM products WHERE (selling_type = 'bundle' OR name LIKE '%bundle%' OR description LIKE '%bundle%')";
        let params: any[] = [];
    ...
    <div className="w-full pb-24 text-zinc-300 min-h-screen">
      <div className="sticky top-0 z-40 bg-black/80 backdrop-blur-xl border-b border-zinc-800/60 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-white font-bold text-lg">
          {search ? `${t('search')}: ${search}` : category ? `${t('category')}: ${category}` : dir === 'rtl' ? 'الباقات والعروض' : 'Bundles & Offers'}
        </h1>
      </div>
