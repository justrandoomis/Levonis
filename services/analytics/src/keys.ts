 ...
 * refused as `forged`. For Analytics that is a deliberate loss of telemetry
 * rather than a deliberate loss of evidence: a number attributed to a producer
 * nobody can identify is not a number worth having, and the events stay in the
 * producers' outboxes to be replayed once the keys are registered.
 */
...
async function build(env: Env, nowMs: number): Promise<Cached> {
  ...
  try {
    ...
    console.error('analytics: ALLOWED_CALLER_KIDS could not be parsed:', e instanceof Error ? e.message : String(e));
    ring = new KeyRing();
  }
  if (env.IDENTITY?.getPublicKeys) {
    try {
      ...
      console.warn('analytics: IDENTITY.getPublicKeys failed, using the bootstrap allowlist only:', e instanceof Error ? e.message : String(e));
    }
  }
