 ...
 * one text (`src/schema.ts` → `migrations/0001_analytics_init.sql`), the
 * migration is additive and re-runnable, and it creates exactly the tables the
 * design gives to Analytics plus this consumer's platform tables.
 */
...
import { analyticsSchemaSql } from '../src/schema';
...
test('the migration is the schema module, statement for statement', () => {
  assert.deepEqual(statements(migrationSql()), statements(analyticsSchemaSql()));
});
...
test('applying the migration twice leaves the same schema and no rows', () => {
  ...
  assert.equal(count(raw, 'SELECT COUNT(*) AS n FROM analytics_daily_platform'), 0);
  assert.equal(count(raw, 'SELECT COUNT(*) AS n FROM analytics_daily_merchant'), 0);
});

test('it creates only tables the design gives to Analytics (or this service own platform tables)', () => {
  const raw = new DatabaseSync(':memory:');
  raw.exec(migrationSql());
  for (const t of tables(raw, 'table')) {
    if (t.startsWith('sqlite_')) continue;
    const owner = ownerOfTable(t) ?? platformTableOwner(t);
    assert.equal(owner, 'analytics', `${t} is owned by ${owner ?? 'nobody'}, not analytics`);
  }
});
...
test('the service keeps its own migration stream, separate from the core stream', () => {
  ...
  assert.ok(core.length > 0 && !core.includes(files[0]), 'the analytics migration is not part of the core stream');
});
