 ...
 *
 * The schema under test is `migrations/0001_analytics_init.sql` read from disk,
 * not a hand-written copy: a test that passes against a schema the deploy never
 * applies proves nothing. `batch()` is a transaction with the same
 * all-or-nothing semantics D1 gives, which is what makes "a redelivery adds
 * nothing to a rollup" testable at all.
 */
...
export const migrationSql = (file = '0001_analytics_init.sql'): string => readFileSync(join(SERVICE_ROOT, 'migrations', file), 'utf8');
...
export function analyticsDb(): { db: D1Database; raw: DatabaseSync } {
