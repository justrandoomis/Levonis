...
export const ROUTES: readonly RouteRule[] = [
  ...
  { prefix: '/api/merchant', hosts: 'root', owner: 'MARKETPLACE', flipPhase: 6, requires: 'auth', rateClass: 'user', note: 'covers /api/merchant/store/layout — the store page draft, publish, history and restore (merchant platform W2-C) — and /api/merchant/notifications/*, /api/merchant/inbox, /api/merchant/analytics/report (the store notification centre, inbox and range analytics, W2-E); /api/merchant/finance and /api/merchant/payouts (the merchant ledger, finance summary and payout requests, W2-B); /api/merchant/products* and /api/merchant/collections* (worker/routes/merchantCatalog.ts — the catalogue, variants, bulk, import/export, collections, W2-F); /api/merchant/attention and /api/merchant/search (worker/routes/merchantWorkspace.ts — the workspace Command Center counts and the command-palette search, owner-scoped, W3-A); /api/merchant/customers (worker/routes/merchantCustomers.ts — the paged, searchable customer list and one customer with their orders here) and /api/merchant/orders/:id/timeline (worker/routes/merchantOrders.ts — one order\'s events, ledger lines and delivery snapshot), owner-scoped, W3-B; /api/merchant/workshop/* (worker/routes/merchantWorkshop.ts — the «مناسب لي» board, live eligibility verdicts and private request costing) and /api/merchant/material-stock (the workshop stock), owner-scoped, W5-B' },
