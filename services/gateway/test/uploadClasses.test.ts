...
test('the admin JSON allowance covers the template body the core actually accepts', () => {
  ...
  assert.ok(bytes && bytes > 1_000_000);
  assert.equal(maxBodyBytes('/api/admin/template/apply', 'POST'), bytes, 'a 1 MB flat cap would have refused a real admin save at the edge');
  assert.ok(JSON_CLASSES.some((r) => r.prefix === '/api/admin'), 'admin mutations are authenticated, apex-only and admin-write limited');
});

test('every class remains within the explicit 48 MiB application envelope limit', () => {
