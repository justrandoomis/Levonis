 ...
 *
 * AND BUMP IT WHENEVER THE PRECACHED BYTES CHANGE, which is the second reason
 * and the one that had already gone wrong once. PRECACHE_URLS is the only list
 * here whose entries are NOT content-hashed: `/icons/icon-192.png` names the
 * same URL forever while the bytes behind it are regenerated by hand from the
 * shop logo (scripts/build-pwa-icons.mjs, and see src/lib/siteLogo.ts on why
 * those PNGs are a fork that no upload to R2 ever reaches). Four of the seven
 * are then never re-requested by anything: the document links only
 * apple-touch-icon and the two favicons, so only those three ever take the
 * stale-while-revalidate refresh below. `icon-192`, `icon-512` and both
 * maskables are read by the install prompt and by nothing else, so once they
 * are in `levonis-static-<VERSION>` the sole thing that can evict them is
 * `activate` seeing a cache name that is no longer current — i.e. this line.
 *
 * That is exactly what happened at v1: the seven PNGs were replaced in the
 * repository (367d2d1) after v1 had shipped, this constant was not touched,
 * and every already-installed app went on repainting the previous mark from
 * its own precache no matter how many deploys went out. v2 is that flush.
