...
// UTF-8 JSON envelope cap, shared with the gateway; asset data is ASCII base64.
export const MAX_TEMPLATE_BODY_BYTES = 48 * 1024 * 1024;
export function assertTemplateBodySize(body: unknown): void {
  if (new TextEncoder().encode(JSON.stringify(body)).length > MAX_TEMPLATE_BODY_BYTES) throw badRequest('Template JSON exceeds 48 MiB', 'TEMPLATE_TOO_LARGE');
}
...
export async function prepareTemplateMedia(env: Env, doc: ProductDoc, text: string): Promise<{ doc: ProductDoc; uploads: Upload[] }> {
  ...
  for (const key of used) if (!seen.has(key) && !(await getMediaObject(env, 'public', key))) throw badRequest(`Missing internal media ${key}; use the complete export or upload it again`, 'MEDIA_MISSING');
