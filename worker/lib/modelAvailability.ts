...
function pricing(raw: unknown, path: string): FulfillmentPricing {
  ...
  if (out.image && !safeLink(out.image)) throw badRequest(`${path}.image: invalid URL`);
...
export function mergeLegacyModels(options: OptionV2[]): { options: OptionV2[]; aliases: Map<string, string> } {
  ...
  for (const rows of byKey.values()) {
    ...
    // Legacy rows priced a whole fulfillment against the product. Move those
    // fields once, without also charging them as a model delta. Their exact
    // values (including cost/stock/image/SKU) remain on the route rows.
    for (const key of PRICE_KEYS) model[key] = null;
    merged.push(model);
  }
