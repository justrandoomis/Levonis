...
import { deleteMediaObject, headMediaObject, isSafeMediaKey } from './mediaStorage';
...
const MEDIA_COLUMNS = /(?:image|media|file|attachment|avatar|cover|logo|photo|video|asset|url|snapshot|content|options|colors|usage_guide|ops_policy|^key$)/i;
const NON_REFERENCES = new Set(['file_objects', 'file_migration_log', 'product_deletion_jobs', 'media_cleanup_jobs', 'product_orphan_reports', 'audit_log', 'core_audit_details', 'historical_inventory_ledger', 'media_cleanup_locks']);
...
export async function processProductMediaCleanup(env: Env, jobId?: string, limit = 30): Promise<{ deleted: number; shared: number; pending: number }> {
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      await deleteMediaObject(env, job.visibility === 'private' ? 'private' : 'public', key);
      if (await headMediaObject(env, job.visibility === 'private' ? 'private' : 'public', key)) throw new Error('R2 object still exists after deletion');
      await env.DB.batch([
...
export async function deleteProductPermanently(env: Env, id: string, actorId: string): Promise<ProductDeleteReport> {
  ...
  try { clean = await processProductMediaCleanup(env, jobId, 50); } catch { /* committed outbox is retried by scheduled jobs */ }
