  ...
  for (const row of results) {
    ...
    const availability = saleAvailability(doc, {
      ...
      inventory: view
        ...
        : undefined,
      links: view?.links,
      preferredType: sel.fulfillmentType ?? (String(row.transport_method ?? '') ? 'pre_order' : null),
    });
    items.push({
      id: row.cart_item_id,
      productId: row.id,
      ...
      transport_method: row.transport_method,
      fulfillment_type: resolved.selection_snapshot?.fulfillment_type,
      selection_snapshot: { ...resolved.selection_snapshot, product_id: String(row.id), option_id: sel.optionId || null, variant_key: doc.options.find((o) => o.id === sel.optionId)?.variant_key ?? '', local_delivery_method: row.local_delivery_method ?? null },
  ...
  const existingLine = await c.env.DB
    .prepare(
      `SELECT id, warranty_plan_id FROM cart_items
        WHERE user_id = ? AND product_id = ? AND option_id = ? AND color_id = ? AND shipping_method_id = '' AND fulfillment_type=? AND transport_method=?`
    )
    .bind(user.id, productId, primaryOption, colorId, fulfillmentType, transportMethod)
...
cartRoutes.patch('/items/:id', async (c) => {
  ...
  const fulfillmentType = oneOf(body.fulfillmentType ?? (body.transportMethod !== undefined ? (transportMethod ? 'pre_order' : 'direct_sale') : existing.fulfillment_type ?? (transportMethod ? 'pre_order' : 'direct_sale')), 'fulfillmentType', ['direct_sale', 'pre_order'] as const);
