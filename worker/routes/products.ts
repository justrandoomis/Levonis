...
// Read on the primary D1 for every anonymous catalog cache lookup. Never
// cached itself: all POPs must observe deletion's committed generation.
productRoutes.get('/cache-generation', async (c) => {
  c.header('Cache-Control', 'no-store');
  const row = await c.env.DB.prepare('SELECT revision FROM catalog_revision WHERE id=1').first<{revision:number}>();
  return c.json({ success: true, revision: row?.revision ?? 0 });
});
...
export function saleAvailability(
  ...
  const preorderAvailable = input.inventory && !isComposition ? resolveStock(input.inventory, { option_value_ids: selectedValueIds, color_id: color?.id ?? null, fulfillment_type: 'pre_order' }).available : null;
  const preorderUsable = preorderEnabled && (preorderAvailable === null || preorderAvailable > 0) && transports.some((t) => t.configured);
  const preorderReason = preorderEnabled
    ? preorderUsable
      ? null
      : preorderAvailable === 0 ? 'OUT_OF_STOCK' : transports.length === 0
  ...
  const maxQty =
    mode === 'unavailable'
      ...
      : isComposition
        ...
        : mode === 'preorder' ? Math.min(QTY_CEILING, preorderAvailable ?? QTY_CEILING) : available === null
          ? QTY_CEILING
