...
| COD | `POST /api/orders` with `paymentMethodId: 'cash'`; collection via `POST /api/orders/:id/settlement` | nothing collected at creation; collection is a separate audited event. **Pricing (decision row 90):** a direct cart is priced the same by either method; a pre-order cart paid `cash` is priced as a DIRECT sale (base + `direct_surcharge_iqd`, no commission; an active PRO is exempt from both) while `shipping_type` stays `preorder_*` with its fourteen stages — `worker/lib/paymentPolicy.ts`, `pricing_snapshot.pricing_basis` | admin records the collection | v4 PAY-03 (points appear only after a recorded collection), PTS-02; `tests/checkoutPayment.test.ts` | implemented |
...
| COD eligibility per product class / community store | `POST /api/orders/quote` → `allowed_payment_methods` | `worker/lib/paymentPolicy.ts`: `wallet` + `cash` for every shipping type; `half_advance` refused (`PAYMENT_METHOD_NOT_ALLOWED`), `full_advance` tolerated as an alias | owner | `tests/checkoutPayment.test.ts` (offered list echoed; half_advance refused on quote and order) | implemented for the platform cart (decision row 90); the community-store side keeps its own `cod`/`wallet` vocabulary and is not covered by that row |
...
| 3 | COD: which order classes and community stores may use it | row 90 (platform cart: wallet or cash for every shipping type; a COD pre-order is priced as a direct sale and stays a pre-order); community stores still pending | one server-side payment computation for every path; digital subscriptions have no COD branch |
