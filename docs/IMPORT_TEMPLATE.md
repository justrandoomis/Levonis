> تحديث البنية: الصيغة الحالية تفصل الموديل عن التوفر والنقل. استخدم
> `options.N.direct.*` و`options.N.preorder.*` و`options.N.preorder.transports.M.*`.
> مقاطع `availability_type` أدناه توثق التوافق مع الملفات القديمة فقط؛ لم تعد
> صيغة إنشاء صفوف التوفر. انظر [التقرير الحالي](PRODUCT_ARCHITECTURE.md).
