...
أضيفت بوابة إصدار فعلية في `scripts/product-release-preflight.mjs`: تفحص بأوامر SELECT فقط سجل migrations والأعمدة المطلوبة وصفوف legacy المتعارضة وأعداد التاريخ. البناء التلقائي في Cloudflare Workers Builds يرفض متابعة النشر إن لم تكن D1 جاهزة؛ لا يشغّل migrations أو cleanup تلقائيًا. مسار النشر الموجود `7 - Deploy LIVE main site levonis-staging` يحتفظ بتأكيد `DEPLOY-CODE`، ويجري preflight قبل الترحيل، ويسجل D1 Time Travel recovery bookmark، ثم يفحص المخطط بعد الترحيل ويحفظ التقريرين. نافذة الصيانة وإيقاف الشراء/تحرير المنتجات أثناء التحويل تظل مسؤولية تشغيلية يجب ترتيبها قبل تشغيل هذا المسار؛ لا توجد نافذة صيانة تلقائية في هذا التغيير. هذا فحص جاهزية إصدار وليس dry-run لبقايا R2.
...
يشترط اختيار النموذج الأساسي وجود مجموعة خيارات تابعة للمنتج نفسه. صف legacy ذو مجموعة مفقودة أو تابعة لمنتج آخر لا يدخل في الدمج أو مرآة JSON أو قراءة الكتالوج، حتى لو شارك النموذج السليم في `variant_key`. يبقى الصف الأصلي دون تعديل ويظهر في تقرير orphan. اختبار ترحيل ممتلئ يثبت ذلك مع حفظ أسعار Direct والحجوزات والـaliases، ويتحقق من قراءة صفحة المنتج والقائمة معًا.
...
جميع تغييرات كتالوج المنتج ترفع `catalog_revision` في D1. gateway يقرأ revision عبر مسار غير مخزن في الكاش قبل استخدام cache المنتجات/البحث/القوائم/الباقات/الملفات، ويضعه في مفتاح التخزين. بذلك تصبح المفاتيح القديمة غير قابلة للوصول في جميع نقاط Cloudflare، حتى لو لم يمكن مسحها فعليًا من POP آخر. اختبار بمخزنين منفصلين يثبت عدم إعادة ghost product. تُمسح مفاتيح Cache API المحلية المعروفة أيضًا، وتستخدم ملفات المنتج `no-store` للمتصفح. إذا كان gateway ضمن المسار الفعلي فيجب تحديثه مع core. إعدادات المستودع تصف gateway الحالي كـdark Worker وتصف `levonis-staging` بأنه يخدم الموقع؛ لم يُنفّذ تحويل مسارات أو إنشاء gateway حي ضمن هذا العمل، ويلزم التحقق من bindings الفعلية قبل النشر.
...
| اختبارات الجذر | 2,573 ناجحًا، صفر فشل أو تخطٍّ |
| اختبارات workspaces | 362 ناجحًا، صفر فشل أو تخطٍّ |
| المجموع | **2,935 اختبارًا ناجحًا** |
...
الجولة الكاملة الأولى استخدمت نفس ملفات root وworkspaces التي يستدعيها `test:unit`، مع `--test-concurrency=6` وسجل خروج مستقل لكل مجموعة؛ الجولة الافتراضية السابقة توقفت دون ملخص ولم تُحتسب ضمن النتائج. أصبح `npm run test:unit` نفسه يحدد concurrency=6 ويحمّل منع الشبكات الخارجية تلقائيًا لكلتا المجموعتين؛ لا يحتاج نجاح الاختبارات إلى تذكر هذا الإعداد يدويًا.

اختبارات التغيير تشمل فصل النموذج/التنفيذ، أسعار العضويات وPRO waiver، override صفر، النقل والـlead time، المخزون والتحقق في checkout، ترحيل legacy ممتلئ وعزل مجموعات الخيارات غير السليمة، حذف graph كامل، shared images، فشل R2 وإعادة المحاولة، فشل D1 والكتابة المتزامنة، orphan dry-run/confirmation، حماية historical images، وحذف نسخ buckets القديمة.
